...
// Solo admin: Usuarios, Materiales, Compradores, Ajustes
// ============================================================
...
function renderConfiguracion() {
  if (SESSION.rol !== 'Admin') {
    document.getElementById('main-content').innerHTML = `
      <div class="card" style="text-align:center;padding:60px">
        ...
        <div style="font-size:13px;color:var(--tl);margin-top:6px">Solo los administradores pueden acceder</div>
      </div>`;
    return;
  }
  ...
  document.getElementById('main-content').innerHTML = `
    <div class="tabs-bar">
      <button class="tab-btn active" id="cfg-tab-usuarios" onclick="switchConfigTab('usuarios')">
      ...
      <button class="tab-btn" id="cfg-tab-ajustes" onclick="switchConfigTab('ajustes')">
        <i class="ti ti-settings-2"></i> Ajustes
      </button>
    </div>
  ...
  document.getElementById('config-tab-content').innerHTML = `
    <div class="config-section">
      <div class="config-section-head">
        ...
        <button class="btn btn-primary btn-sm" onclick="abrirFormUsuario()"><i class="ti ti-plus"></i> Nuevo</button>
      </div>
      <div style="padding:30px;text-align:center"><div class="spinner" style="margin:0 auto"></div></div>
    </div>`;
  try {
    const res = await apiGet({ action: 'getUsuariosTodos' });
    USUARIOS_DATA = res.ok ? res.data : [];
    renderTablaUsuarios();
  } catch(e) { showToast('Error cargando usuarios'); }
}
    ...
    <div class="config-section">
      ...
      <div class="config-section-body" style="padding:0">
        <div class="table-wrap" style="border:none;border-radius:0"><table>
          ...
          <tbody>${USUARIOS_DATA.length ? USUARIOS_DATA.map(u => `
            <tr>
              ...
              <td><div class="td-actions"><button class="btn btn-primary btn-sm" onclick="abrirFormUsuario('${u['ID_Usuario']}')"><i class="ti ti-pencil"></i></button></div></td>
            </tr>`).join('') : '<tr><td colspan="5" style="text-align:center;color:var(--tl);padding:30px">No hay usuarios</td></tr>'}
          </tbody>
        </table></div>
      </div>
    </div>`;
      ...
      <div class="modal-body">
        ...
        <div class="form-group">
          ...
          <input type="email" class="form-input" id="usr-email" value="${u?.['Email']||''}"
            placeholder="nombre@redesconrostro.org" ${u?'readonly style="background:#f8fbfd;color:var(--tm)"':''}>
...
async function guardarUsuario(id) {
  ...
  try {
    const res = await apiPost({ action: 'saveUsuario', data: { ID_Usuario:id||'', Nombre:nombre, Email:email, Rol:rol, Activo:activo }});
...
function renderMateriales() {
  document.getElementById('config-tab-content').innerHTML = `
    <div class="config-section">
      <div class="config-section-head">
        <div class="config-section-title"><i class="ti ti-recycle"></i> Materiales <span class="badge badge-blue">${CAT.materiales.length}</span></div>
        <button class="btn btn-primary btn-sm" onclick="abrirFormMaterial()"><i class="ti ti-plus"></i> Nuevo</button>
      </div>
      <div class="config-section-body" style="padding:0">
        <div class="table-wrap" style="border:none;border-radius:0"><table>
          <thead><tr><th>Nombre</th><th>Priorizable</th><th>Activo</th><th></th></tr></thead>
          <tbody>${CAT.materiales.length ? CAT.materiales.map(m => `
            <tr>
              <td style="font-weight:500">${m['Nombre']}</td>
              <td>${m['Priorizable']==='Sí'||m['Priorizable']===true?'<span class="badge badge-cyan">Sí</span>':'<span style="color:var(--tl);font-size:12px">No</span>'}</td>
              ...
              <td><div class="td-actions"><button class="btn btn-primary btn-sm" onclick="abrirFormMaterial('${m['ID_Material']}')"><i class="ti ti-pencil"></i></button></div></td>
            </tr>`).join('') : '<tr><td colspan="4" style="text-align:center;color:var(--tl);padding:30px">No hay materiales</td></tr>'}
          </tbody>
        </table></div>
      </div>
    </div>`;
    ...
    <div class="modal" style="max-width:400px">
      ...
      <div class="modal-body">
        <div class="form-group"><label class="form-label">Nombre *</label>
          <input type="text" class="form-input" id="mat-nombre" value="${m?.['Nombre']||''}"></div>
        <div class="form-grid-2">
          <div class="form-group"><label class="form-label">Priorizable</label>
...
async function guardarMaterial(id) {
  ...
  if (btn) { btn.disabled=true; btn.innerHTML='<i class="ti ti-loader"></i> Guardando...'; }
  try {
    const res = await apiPost({ action:'saveMaterial', data:{ ID_Material:id||'', Nombre:nombre, Priorizable:prio, Activo:activo }});
    if (!res.ok) { showToast('Error: '+(res.error||'desconocido')); return; }
...
function renderCompradores() {
  document.getElementById('config-tab-content').innerHTML = `
    <div class="config-section">
      <div class="config-section-head">
        <div class="config-section-title"><i class="ti ti-truck"></i> Compradores <span class="badge badge-blue">${CAT.compradores.length}</span></div>
        <button class="btn btn-primary btn-sm" onclick="abrirFormComprador()"><i class="ti ti-plus"></i> Nuevo</button>
      </div>
      <div class="config-section-body" style="padding:0">
        <div class="table-wrap" style="border:none;border-radius:0"><table>
          <thead><tr><th>Nombre</th><th>Nivel</th><th>Destino final</th><th>Provincia</th><th>Activo</th><th></th></tr></thead>
          <tbody>${CAT.compradores.length ? CAT.compradores.map(c => `
            <tr>
              ...
              <td><div class="td-actions"><button class="btn btn-primary btn-sm" onclick="abrirFormComprador('${c['ID_Comprador']}')"><i class="ti ti-pencil"></i></button></div></td>
            </tr>`).join('') : '<tr><td colspan="6" style="text-align:center;color:var(--tl);padding:30px">No hay compradores</td></tr>'}
          </tbody>
        </table></div>
      </div>
    </div>`;
...
function abrirFormComprador(id = null) {
  ...
  abrirModal(`
    <div class="modal">
      ...
      <div class="modal-body">
        <div class="form-group"><label class="form-label">Nombre *</label>
          <input type="text" class="form-input" id="com-nombre" value="${c?.['Nombre']||''}"></div>
        <div class="form-grid-2">
          <div class="form-group"><label class="form-label">Nivel intermediación</label>
          ...
          <div class="form-group"><label class="form-label">Provincia</label>
            <input type="text" class="form-input" id="com-provincia" value="${c?.['Provincia']||''}"></div>
        </div>
        <div class="form-group"><label class="form-label">Destino final</label>
          <input type="text" class="form-input" id="com-destino"
            placeholder="Ej: Se vende a INTERCIA S.A." value="${c?.['Destino Final']||''}"></div>
        <div class="form-group"><label class="form-label">Activo</label>
...
async function guardarComprador(id) {
  ...
  try {
    const res = await apiPost({ action:'saveComprador', data:{
      ID_Comprador:id||'', Nombre:nombre, 'Nivel Intermediacion':nivel,
...
function renderAjustes() {
  document.getElementById('config-tab-content').innerHTML = `
    <div class="config-section">
      <div class="config-section-head">
        <div class="config-section-title"><i class="ti ti-settings-2"></i> Ajustes generales</div>
      </div>
      <div class="config-section-body">
        <div class="form-group">
          <label class="form-label">ID Carpeta raíz Drive</label>
          <input type="text" class="form-input" value="${DRIVE_FOLDER_ROOT}" readonly
            style="background:#f8fbfd;color:var(--tm);font-size:12px;font-family:monospace">
          <div class="form-hint">Carpeta raíz donde se organizan las evidencias por asociación.</div>
        </div>
        <div style="margin-top:8px">
          <a href="https://drive.google.com/drive/folders/${DRIVE_FOLDER_ROOT}"
            target="_blank" class="btn btn-glass btn-sm">
            <i class="ti ti-folder"></i> Abrir en Drive
          </a>
        </div>
        <div style="margin-top:24px;padding-top:20px;border-top:1px solid var(--border)">
          <div class="form-label" style="margin-bottom:6px">Apps Script URL</div>
          <div style="font-size:11px;font-family:monospace;color:var(--tm);word-break:break-all;
            background:#f8fbfd;padding:10px;border-radius:var(--radius-sm);border:1px solid var(--border)">
            ${SCRIPT_URL}
          </div>
        </div>
        <div style="margin-top:24px;padding-top:20px;border-top:1px solid var(--border)">
          <div style="font-size:13px;font-weight:600;color:var(--tm)">ReCircula 360 · v1.0 · 2026</div>
          <div style="font-size:12px;color:var(--tl);margin-top:2px">Redes Con Rostro</div>
        </div>
      </div>
    </div>`;
}
