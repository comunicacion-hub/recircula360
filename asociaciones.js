...
const buscarAsociacionDebounced = debounce(buscarAsociacion, 250);
...
function renderTablaAsociaciones() {
  ...
  const filas = data.map(a => {
    ...
    return `
      <tr>
        <td style="font-weight:600;color:var(--tx)">${nombreSafe || '—'}</td>
      ...
      <div class="modal-body">
        <div class="form-grid-2">
          ...
          <div>
            ...
            <div style="font-size:20px;font-weight:700;color:var(--tx);margin-top:4px">
...
async function guardarAsociacion(id) {
  ...
  try {
    ...
    const res = await apiPostQueued({ action: 'saveAsociacion', data });
